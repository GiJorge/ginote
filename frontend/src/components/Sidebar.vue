<template>
  <aside :class="['fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0', isOpen ? 'translate-x-0' : '-translate-x-full']">
    ...
    <div class="overflow-y-auto flex-1 p-3 space-y-2">
      <div v-for="n in filteredNotes" :key="n.id" @click="$emit('select-note', n)"
           :class="['p-4 rounded-xl cursor-pointer', activeId === n.id ? 'bg-indigo-600 text-white' : 'hover:bg-slate-100 dark:hover:bg-slate-800']">
